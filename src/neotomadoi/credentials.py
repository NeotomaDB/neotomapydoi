...
class credentials:
    ...
    def __init__(self, datacite_meta: dict):
        ...
        """
        assert isinstance(datacite_meta, dict), (
            "You must pass a `dict` as the metatdata."
        )
        assert all([i in datacite_meta.keys() for i in ["user", "mode"]]), (
            "Your client metadata must be a dict with the keys `user`, and `mode`."
        )
        assert all([i in datacite_meta.get("mode").keys() for i in ["test", "prod"]]), (
